...
public abstract partial class SharedStainSystem : EntitySystem
{
    ...
    private void OnSpilledOn(Entity<StainableComponent> ent, ref InventoryRelayedEvent<SpilledOnEvent> args)
    {
        ...
        // Ganimed-Edit: не отбираем у источника больше, чем влезет в пятно: иначе
        // TryAddSolution откажет, а реагенты уже будут удалены из лужи и пропадут.
        ...
        // Ganimed-Edit: добавляем раствор в пятно только если он реально влез
        // (иначе визуал обновлялся, хотя реагенты не перенеслись).
