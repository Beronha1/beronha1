...
public sealed partial class BloodCultWeaponSystem : EntitySystem
{
    ...
    private void OnInteractEvent(EntityUid uid, BloodCultWeaponComponent component, AfterInteractEvent args)
    {
        ...
        if (_body.TryGetOrgansWithComponent<StomachComponent>((args.Target.Value, body), out var stomachs))
        {
            ...
            // 🌇Sunset🌇 - this fork's StomachSystem has no TryChangeReagent helper; reuse the same
            // in-place solution mutation ConvertHolyWater already does for the injectable-solution path.
            if (ConvertHolyWater(stomachSol, component.ConvertedId, component.ConvertedToId))
                convert = true;
        }
