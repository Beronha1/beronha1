...
ghost-gui-ghost-warp-button = Deslocamento fantasma
ghost-gui-ghost-roles-button = Funcoes de fantasma ({$count})
...
ghost-target-window-title = Deslocamento fantasma
ghost-target-window-current-button = Deslocar: {$name}
...
ghost-roles-window-leave-raffle-button =
    Sair da raffle ({$time} restante, { $players ->
         [one] {$players} player
...
ghost-roles-window-no-roles-available-label = Atualmente nao ha funcoes de fantasma disponiveis.
...
ghost-return-to-body-text = A equipe médica está mandando bem e você está sendo revivido! Quer voltar ao seu corpo?
