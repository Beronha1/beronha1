...
public sealed partial class ESCluesSystem : EntitySystem
{
    ...
    public string GetHairColorClue(Entity<ESCharacterComponent?> mind)
    {
        ...
        foreach (var organMarkings in mind.Comp.Profile.Appearance.Markings.Values)
        {
            if (!organMarkings.TryGetValue(HumanoidVisualLayers.Hair, out var hair) ||
                hair.Count == 0 ||
                hair[0].MarkingColors.Count == 0)
                continue;

            color = hair[0].MarkingColors[0];
            break;
        }

        return Loc.GetString("es-clue-hair-fmt", ("color", GetHairColorString(color)));
    }
