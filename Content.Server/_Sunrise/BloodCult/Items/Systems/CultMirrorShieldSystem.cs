...
public sealed partial class CultMirrorShieldSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        // 🌇Sunset🌇 - this fork's Content.Shared.Blocking.BlockingSystem doesn't raise a per-hit
        // BlockingEvent (damage modifiers are applied directly, not via an event other systems can hook),
        // so the "shield can break/spawn an illusion from a blocked melee hit" path has no equivalent
        // here - only the hitscan-reflect path (OnHitScanReflected) is wired up.
