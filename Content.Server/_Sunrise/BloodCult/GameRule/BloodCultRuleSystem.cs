...
public sealed partial class BloodCultRuleSystem : GameRuleSystem<BloodCultRuleComponent>
{
    ...
    public override void Initialize()
    {
        ...
        // 🌇Sunset🌇 - this fork has no CryostorageEnteredEvent to hook "target went to cryo, needs
        // replacing" (OnCultTargetDeleted's ComponentShutdown path still covers targets whose *entity*
        // gets deleted/gibbed, just not the cryo-specific case).

        SubscribeLocalEvent<BloodCultRuleComponent, AfterAntagEntitySelectedEvent>(AfterEntitySelected);
        // 🌇Sunset🌇 - no AntagSelectionCompleteEvent in this fork either; GameRunLevelChangedEvent ->
        // InRound (fires once round-start spawning finishes) is the equivalent "antag selection is done,
        // everyone's spawned in" hook, same pattern used by NukeopsRuleSystem.
    ...
    private void OnRunLevelChanged(GameRunLevelChangedEvent ev)
    {
        ...
        {
            if (!ruleComp.TargetsPopulated)
                PopulateCultTargets((uid, ruleComp));
        }
    }

    private void PopulateCultTargets(Entity<BloodCultRuleComponent> ent)
    {
        ent.Comp.TargetsPopulated = true;
    ...
    private void RemoveCultistAppearance(EntityUid cultist)
    {
        if (HasComp<HumanoidProfileComponent>(cultist))
            _humanoidProfile.SetEyeColor(cultist, Color.White);
    ...
    private void UpdateCultistsAppearance(UpdateCultAppearance ev)
    {
        ...
        {
            if (HasComp<HumanoidProfileComponent>(cultist))
                _humanoidProfile.SetEyeColor(cultist, rule.EyeColor);
    ...
    private void OnNarsieSummon(CultNarsieSummoned ev)
    {
        ...
        // 🌇Sunset🌇 - this fork has no ForceSetCountdown; RequestRoundEnd(countdownTime, ..., cantRecall)
        // is the equivalent "force the shuttle countdown to this duration" call, bypassing the normal
        // call-shuttle cooldown since this is a forced summon-completion trigger, not a player request.
        _roundEndSystem.RequestRoundEnd(TimeSpan.FromSeconds(10), checkCooldown: false, cantRecall: true);
        // Sunrise edit end
    }
}
