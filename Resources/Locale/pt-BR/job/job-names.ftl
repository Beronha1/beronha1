# SPDX-FileCopyrightText: 2026 punkzebub <punkzebub@gmail.com>
#
# SPDX-License-Identifier: AGPL-3.0-or-later

job-name-atmostech = Técnico atmosférico
job-name-bartender= Barman
job-name-borg = Ciborgue
job-name-botanist = Botânico
job-name-brigmedic = Médico da brig
job-name-cadet = Cadete de segurança
job-name-captain = Capitão
job-name-cargotech = Técnico de carga
job-name-cburn = Oficial de quarentena da CentComm
job-name-ce = Engenheiro-chefe
job-name-centcommoff = Oficial da CentComm
job-name-chef= Cozinheiro
job-name-chaplain = Capelão
job-name-chemist = Químico
job-name-clown = Palhaço
job-name-cluwne= Dica
job-name-cmo = Diretor médico
job-name-deathsquad = Esquadrão da morte
job-name-detective = Detetive
job-name-doctor = Médico
job-name-engineer = Engenheiro da estação
job-name-ertchaplain = Capelão da ERT
job-name-ertengineer = Engenheiro da ERT
job-name-ertjanitor = Zelador da ERT
job-name-ertleader = Líder da ERT
job-name-ertmedic = Médico da ERT
job-name-ertsecurity = Segurança da ERT
job-name-hop = Chefe de pessoal
job-name-hos = Chefe de segurança
job-name-intern = Interno de medicina
job-name-janitor = Zelador
job-name-lawyer = Advogado
job-name-librarian = Bibliotecário
job-name-mime = Mímico
job-name-musician = Músico
job-name-paramedic = Paramédico
job-name-passenger = Auxiliar
job-name-psychologist = Psicólogo
job-name-qm = Intendente
job-name-rd = Diretor de pesquisa
job-name-reporter = Repórter
job-name-research-assistant = Assistente de pesquisa
job-name-salvagespec = Especialista em salvamento
job-name-scientist = Cientista
job-name-security = Oficial de segurança
job-name-serviceworker = Trabalhador de serviços
job-name-station-ai = IA da estação
job-name-technical-assistant = Assistente técnico
job-name-visitor = Visitante
job-name-warden = Carcereiro

# Funções não usadas
# Estas são necessárias para as dicas dos ícones de função nos cartões de identificação de agentes.
# Elas são mantidas para oportunidades de interpretação.
job-name-geneticist = Geneticista
job-name-boxer = Boxeador
job-name-no-id = Sem ID
job-name-prisoner = Prisioneiro
job-name-roboticist = Roboticista
job-name-unknown = Desconhecido
job-name-virologist = Virologista
job-name-zookeeper = Tratador de zoológico

# Funções de antagonista
job-name-ninja = Ninja
job-name-syndicate= Sindicato
job-name-syndicate-commander = Comandante do Syndicate
job-name-syndicate-corpsman = Socorrista do Syndicate
job-name-syndicate-operative = Operativo do Syndicate
job-name-pirate = Pirata
job-name-wizard = Mago
job-name-zombie = Zumbi

# Títulos de função
job-title-cluwne= Dica
job-title-universal = Universal
job-title-visitor = Visitante

# Temporizadores de função — mantenha em ordem alfabética
JobAtmosphericTechnician = Técnico atmosférico
JobBartender= Barman
JobBorg = Borg
JobBotanist = Botânico
JobCaptain = Capitão
JobCargoTechnician = Técnico de carga
JobCentralCommandOfficial = Oficial do Comando Central
JobChaplain = Capelão
JobChef= Cozinheiro
JobChemist = Químico
JobChiefEngineer = Engenheiro-chefe
JobChiefMedicalOfficer = Diretor médico
JobClown = Palhaço
JobDetective = Detetive
JobBrigmedic = Médico da brig
JobERTChaplain = Capelão da ERT
JobERTEngineer = Engenheiro da ERT
JobERTJanitor = Zelador da ERT
JobERTLeader = Líder da ERT
JobERTMedical = Médico da ERT
JobERTSecurity = Segurança da ERT
JobHeadOfPersonnel = Chefe de pessoal
JobHeadOfSecurity = Chefe de segurança
JobJanitor = Zelador
JobLawyer = Advogado
JobLibrarian = Bibliotecário
JobMedicalDoctor = Médico
JobMedicalIntern = Interno de medicina
JobMime = Mímico
JobMusician = Músico
JobParamedic = Paramédico
JobPassenger = Auxiliar
JobPsychologist = Psicólogo
JobQuartermaster = Intendente
JobReporter = Repórter
JobResearchAssistant = Assistente de pesquisa
JobResearchDirector = Diretor de pesquisa
JobSalvageSpecialist = Especialista em salvamento
JobScientist = Cientista
JobSecurityCadet = Cadete de segurança
JobSecurityOfficer = Oficial de segurança
JobServiceWorker = Trabalhador de serviços
JobStationAi = IA da estação
JobStationEngineer = Engenheiro da estação
JobTechnicalAssistant = Assistente técnico
JobVisitor = Visitante
JobWarden = Carcereiro
