...
public sealed partial class ESEntityTimerSystem : EntitySystem
{
    ...
    /// <summary>
    /// Returns the timer's elapsed progress on the interval from zero to one.
    /// </summary>
    public float GetTimerProgress(Entity<ESEntityTimerComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp) || ent.Comp.Duration <= TimeSpan.Zero)
            return 0;

        var timeRemaining = ent.Comp.TimerEnd - _timing.CurTime;
        return Math.Clamp(1f - (float) (timeRemaining / ent.Comp.Duration), 0f, 1f);
    }
