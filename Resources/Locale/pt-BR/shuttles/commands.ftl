...
cmd-ftldisk-map-paused = A entidade {$destination} está no mapa {$map} que está pausado! Por favor, despausar o mapa primeiro ou os jogadores ficarão presos no lugar.
cmd-ftldisk-planet = A entidade {$destination} está no mapa do planeta {$map} e precisará de um ponto FTL. Ele pode já existir.
