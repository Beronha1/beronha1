...
using Robust.Shared.Timing; // Ganimed-Edit: таймер самовоспламенения (проверка раз в 2 сек)
...
namespace Content.Server._Funkystation.ReagentFires.Systems
{
    public sealed partial class ReagentFireSystem : EntitySystem
    {
        ...
        [Dependency] private IGameTiming _timing = null!; // Ganimed-Edit: таймер самовоспламенения
        ...
        public override void Update(float frameTime)
        {
            ...
            while (activeQuery.MoveNext(out var uid, out var fireComp, out var puddle, out var xform))
            {
                if (!fireComp.OnFire)
                {
                    ...
                    // Ganimed-Edit: атмосферные запросы на каждую негорящую лужу каждый тик - лишняя
                ...
                // Ganimed-Edit: в вакууме сгорает только самоокисляющаяся часть раствора
                ...
                foreach (var offset in directions)
                {
                    ...
                    // Ganimed-Edit: огонь не проходит сквозь герметичные преграды (закрытые firelock, стены):
                ...
                var structuralProto = _prototypeManager.Index(StructuralDamageType);
                var heatProto = _prototypeManager.Index(HeatDamageType);
