...
public sealed partial class AshDrakeSystem : EntitySystem
{
    ...
    private void OnTerminating(Entity<AshDrakeBossComponent> ent, ref EntityTerminatingEvent args)
    {
    ...
    {
        if (_activeArenas.ContainsKey(ent.Owner) || !Exists(args.Target))
            return;
    ...
    private void StartLavaJump(Entity<AshDrakeBossComponent> ent, AshDrakeLavaActionEvent args)
    {
        ...
        var delta = targetWorldPos - drakeWorldPos;
        var direction = delta == Vector2.Zero ? Vector2.UnitX : Vector2.Normalize(delta);
