# SPDX-FileCopyrightText: 2022 Alex Evgrashin <aevgrashin@yandex.ru>
# SPDX-FileCopyrightText: 2022 KIBORG04 <bossmira4@gmail.com>
# SPDX-FileCopyrightText: 2022 metalgearsloth <comedian_vs_clown@hotmail.com>
# SPDX-FileCopyrightText: 2023 Floofi <126319569+Shadowtheprotogen546@users.noreply.github.com>
# SPDX-FileCopyrightText: 2023 Nemanja <98561806+EmoGarbage404@users.noreply.github.com>
# SPDX-FileCopyrightText: 2023 Veritius <veritiusgaming@gmail.com>
# SPDX-FileCopyrightText: 2023 lapatison <100279397+lapatison@users.noreply.github.com>
# SPDX-FileCopyrightText: 2023 metalgearsloth <31366439+metalgearsloth@users.noreply.github.com>
# SPDX-FileCopyrightText: 2024 Firewatch <54725557+musicmanvr@users.noreply.github.com>
# SPDX-FileCopyrightText: 2024 cool dolphin <36332236+Afrokada@users.noreply.github.com>
# SPDX-FileCopyrightText: 2024 potato1234_x <79580518+potato1234x@users.noreply.github.com>
# SPDX-FileCopyrightText: 2025 Aiden <28298836+Aidenkrz@users.noreply.github.com>
# SPDX-FileCopyrightText: 2025 Aiden <aiden@djkraz.com>
# SPDX-FileCopyrightText: 2025 Alpaccalypse <21291379+Alpaccalypse@users.noreply.github.com>
#
# SPDX-License-Identifier: AGPL-3.0-or-later

reagent-name-coffee =café
reagent-desc-coffee = Uma bebida feita com grãos de café moídos. Contém uma quantidade moderada de cafeína.

reagent-name-cream = creme
reagent-desc-cream = A parte gordurosa e ainda líquida do leite. Por que você não mistura isso com uísque, hein?

reagent-name-coconut-water = água de coco
reagent-desc-coconut-water = Um favorito dos sobreviventes em ilhas desertas.

reagent-name-cream-of-coconut = creme de coco
reagent-desc-cream-of-coconut = Versão doce e xaroposa de creme de coco com adição de açúcar.

reagent-name-cafe-latte = café com leite
reagent-desc-cafe-latte = Uma bebida agradável, forte e saborosa enquanto você lê.

reagent-name-tea-powder = pó de chá
reagent-desc-tea-powder = Folhas de chá finamente moídas, utilizadas para fazer chá.

reagent-name-green-tea = chá verde
reagent-desc-green-tea = Chá verde saboroso.

reagent-name-grenadine = granadina
reagent-desc-grenadine = Não tem sabor de cereja!

reagent-name-iced-coffee = café gelado
reagent-desc-iced-coffee = Café e gelo, refrescante e fresco.

reagent-name-iced-green-tea = chá verde gelado
reagent-desc-iced-green-tea = Chá verde frio.

reagent-name-iced-tea =chá gelado
reagent-desc-iced-tea = Nenhuma relação com um determinado artista/ator de rap.

reagent-name-lemonade = limonada
reagent-desc-lemonade = Beba com suco de limão, água e um adoçante como açúcar de cana ou mel.

reagent-name-arnold-palmer = Arnold Palmer
reagent-desc-arnold-palmer = Agora observe esta viagem.

reagent-name-milk = leite
reagent-desc-milk = Líquido branco opaco produzido pelas glândulas mamárias de mamíferos.

reagent-name-milk-goat = leite de cabra
reagent-desc-milk-goat = Um líquido branco opaco produzido por uma cabra. Alto teor de leitelho.

reagent-name-milk-sheep = leite de ovelha
reagent-desc-milk-sheep = Um líquido branco opaco produzido por uma ovelha. Rico em gordura.

reagent-name-milk-oat = leite de aveia
reagent-desc-milk-oat = Surpreendentemente saboroso.

reagent-name-milk-soy = leite de soja
reagent-desc-milk-soy = Favorito dos consumidores.

reagent-name-milk-spoiled = leite estragado
reagent-desc-milk-spoiled = Este leite ficou rançoso.

reagent-name-nothing = nada
reagent-desc-nothing = Absolutamente nada.

reagent-name-nuclear-cola =cola nuclear
reagent-desc-nuclear-cola = Cola, cola nunca muda.

reagent-name-hot-cocoa = cacau quente
reagent-desc-hot-cocoa = Cheira a férias!

reagent-name-soda-water = água com gás
reagent-desc-soda-water = Um recipiente de club soda. Por que não fazer um uísque com refrigerante?

reagent-name-soy-latte = café com leite de soja
reagent-desc-soy-latte = Uma bebida de café feita com café expresso e leite de soja vaporizado.

reagent-name-tea = chá
reagent-desc-tea = Uma bebida feita fervendo folhas da árvore do chá, Camellia sinensis.

reagent-name-tonic-water = água tônica
reagent-desc-tonic-water = Tem um gosto estranho, mas pelo menos o quinino mantém a malária espacial sob controle.

reagent-name-water = água
reagent-desc-water = Um líquido incolor que os humanos precisam para sobreviver.

reagent-name-ice = gelo
reagent-desc-ice = Água congelada.

reagent-name-dry-ramen = ramen seco
reagent-desc-dry-ramen = Macarrão seco e sal.

reagent-name-hot-ramen = ramen quente
reagent-desc-hot-ramen = Macarrão quente.

reagent-name-pilk =pique
reagent-desc-pilk = Uma mistura enjoativa de leite e cola.

reagent-name-posca = posca
reagent-desc-posca = Bebida dos pobres guerreiros de uma época esquecida.

reagent-name-mopwata= espere
reagent-desc-mopwata = Água suja e estagnada do esfregão.

reagent-name-orange-lime-soda = bicicletário cítrico
reagent-desc-orange-lime-soda = O refrigerante deve ser laranja! Não, deveria ser limão!
