...
public sealed partial class ESRadstormThrusterEngineSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ESRadstormThrusterEngineComponent, SolutionChangedEvent>(OnSolutionChanged);
    ...
    private void OnMapInit(Entity<ESRadstormThrusterEngineComponent> ent, ref MapInitEvent args)
    {
        ...
        if (_solutionContainer.TryGetSolution(ent.Owner, ent.Comp.FuelTankSolutionId, out var solution))
        {
            _solutionContainer.TryAddReagent(solution.Value,
                ...
                _random.Next(ent.Comp.MinStartingFuel.Int(), ent.Comp.MaxStartingFuel.Int()));
        }
    }
    ...
    private void OnSolutionChanged(Entity<ESRadstormThrusterEngineComponent> ent, ref SolutionChangedEvent args)
    {
        if (args.Solution.Comp.Id != ent.Comp.FuelTankSolutionId)
            return;

        if (!_solutionContainer.TryGetSolution(ent.Owner, ent.Comp.FuelTankSolutionId, out var solution))
            return;

        if (args.Solution.Comp.Solution.GetReagentQuantity(new ReagentId(ent.Comp.FuelReagent, null)) != args.Solution.Comp.Solution.Volume)
        {
            var spillage = _solutionContainer.SplitSolutionWithout(solution.Value,
                args.Solution.Comp.Solution.Volume,
                ent.Comp.FuelReagent);
        ...
        ent.Comp.HasFuel = args.Solution.Comp.Solution.Volume > 0;
