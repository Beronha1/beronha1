...
cmd-playtime_getoverall-success = O tempo geral para {$username} é {TOSTRING($time, "dddd\\:hh\\:mm")}.
...
cmd-playtime_getrole-help = Uso: {$command} <user name> [função]
...
cmd-playtime_flush-help = Uso: {$command} [nome de usuário]
    Isso causa uma liberação apenas para o armazenamento interno, mas não para o banco de dados imediatamente.
...
cmd-playtime_flush-arg-user = [nome de usuário]
